...
export const OrderSuccess: React.FC<OrderSuccessProps> = ({
  ...
  return (
    <motion.div
      ...
      className="w-full max-w-2xl mx-auto py-8 px-4 flex flex-col items-center font-body text-[#1b1c1c] dark:text-[#f9fafb]"
    >
      {/* Ambient background glows */}
      <div className="absolute inset-0 pointer-events-none overflow-hidden z-0">
        <div className="absolute top-[-10%] left-[-10%] w-[40%] h-[40%] rounded-full bg-[#ffdad7]/25 dark:bg-[#FF4D4F]/10 blur-[100px]"></div>
        <div className="absolute bottom-[-10%] right-[-10%] w-[30%] h-[30%] rounded-full bg-[#e2e2e4]/35 dark:bg-[#FF4D4F]/5 blur-[80px]"></div>
      </div>

      {/* Header Section */}
      <div className="text-center mb-8 w-full flex flex-col items-center relative z-10">
        <div className="w-20 h-20 rounded-full bg-[#f0eded] dark:bg-white/10 flex items-center justify-center mb-5 shadow-sm border border-white/60 dark:border-white/10">
          <span
      ...
      <div className="glass-panel dark:bg-[#12151c]/75 dark:border-white/10 rounded-2xl p-6 sm:p-8 w-full mb-8 relative z-10 border border-white/70 shadow-lg space-y-5">
        ...
        <div className="flex justify-between items-center pb-4 border-b border-white/60 dark:border-white/10 text-xs">
          <div>
            <p className="text-[10px] text-[#5b403e] dark:text-gray-400 uppercase tracking-widest font-bold">Número de Orden</p>
            <p className="font-mono font-bold text-sm text-[#FF4D4F]">{orderId}</p>
          </div>
          <div className="text-right">
            <p className="text-[10px] text-[#5b403e] dark:text-gray-400 uppercase tracking-widest font-bold">Fecha</p>
            <p className="font-semibold text-xs text-[#1b1c1c] dark:text-[#f9fafb]">
        ...
        {/* Order Items Preview */}
        <div className="space-y-4 py-2 divide-y divide-white/50 dark:divide-white/10">
          {displayItems.map((item, idx) => (
            <div key={item.id + idx} className="pt-3 first:pt-0 flex gap-4 items-center">
              ...
              <span className="text-xs font-bold text-[#1b1c1c] dark:text-[#f9fafb]">${(item.price * item.quantity).toFixed(2)}</span>
            </div>
          ))}
        </div>
        ...
        <div className="space-y-2 pt-4 border-t border-white/60 dark:border-white/10 text-xs text-[#5b403e] dark:text-gray-400">
          <div className="flex justify-between">
            <span>Subtotal</span>
            <span className="font-semibold text-[#1b1c1c] dark:text-[#f9fafb]">${displaySubtotal.toFixed(2)}</span>
          </div>
          <div className="flex justify-between">
            <span>Envío</span>
